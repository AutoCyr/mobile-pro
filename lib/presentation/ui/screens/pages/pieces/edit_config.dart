...
class ConfigEditScreen extends StatefulWidget {
  final PieceInfo detail;
  const ConfigEditScreen({super.key, required this.detail});
...
class _ConfigEditScreenState extends State<ConfigEditScreen> {
  ...
  _loadExistingDisponibilities() async {
    PieceInfo existing = widget.detail;

    setState(() {
      if(existing.autos != null) {
        for(var auto in existing.autos!) {
          autos.add(auto.typeAuto.id);
        }
      }
      if(existing.moteurs != null) {
        for(var moteur in existing.moteurs!) {
          moteurs.add(moteur.typeMoteur.id);
        }
      }
      if(existing.categories != null) {
        for(var category in existing.categories!) {
          Map<String, dynamic> map = {
            "id" : category.categorieEngin.id,
            "hybrid": category.hybride
          };
          categories.add(map);
        }
      }
